...
import { STATUS_COLORS, STATUS_LABELS, formatCurrency } from "./colors";
import { Activity } from "lucide-react";

interface PrStatusDistribution {
  draft: number;
  pending: number;
  inProgress: number;
  approved: number;
...
interface Props {
  data: PrStatusDistribution;
}

export function PrStatusDonut({ data }: Props) {
  const chartData = Object.entries(data)
    .filter(([, value]) => value > 0)
    .map(([key, value]) => ({
      name: STATUS_LABELS[key] || key,
      value,
      color: STATUS_COLORS[key] || "#81899b",
    }));
      ...
      <Card variant="glass" className="overflow-hidden">
        <CardHeader className="pb-2">
          <div className="flex items-center gap-2">
            ...
            <CardTitle className="text-sm font-medium text-fog">وضعیت درخواست‌های خرید</CardTitle>
          </div>
        </CardHeader>
        <CardContent>
