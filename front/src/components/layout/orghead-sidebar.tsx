...
const navSections: NavSection[] = [
  {
    label: "اصلی",
    items: [
      { label: "داشبورد", href: "/orghead", icon: LayoutDashboard },
    ],
  },
  {
    label: "سازمان",
  ...
  },
  {
    label: "فرآیندها",
    items: [
      { label: "فرآیندها", href: "/orghead/processes", icon: Workflow },
    ],
  },
  {
    label: "انبار",
  ...
  },
  {
    label: "تنظیمات",
    items: [
      { label: "سازمان", href: "/orghead/settings", icon: Settings },
    ],
  },
];
...
function OrgHeadSidebar() {
  const [collapsed, setCollapsed] = useState(() => {
    if (typeof window !== "undefined") {
      return localStorage.getItem("orghead-sidebar-collapsed") === "true";
    }
    return false;
  });
  ...
  const handleToggle = useCallback(() => {
    setCollapsed((prev) => {
      const next = !prev;
      localStorage.setItem("orghead-sidebar-collapsed", String(next));
      return next;
    });
  }, []);
...
export { OrgHeadSidebar, OrgHeadMobileNav };
