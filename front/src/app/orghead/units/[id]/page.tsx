    ...
    <div className="space-y-6 max-w-2xl mx-auto">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Link href="/orghead/units" className="text-fog hover:text-moonlight transition-colors">
            <ArrowRight className="size-5" />
          </Link>
          <PageHeader title="ویرایش واحد" description="ویرایش اطلاعات واحد" className="border-none mb-0 pb-0" />
        </div>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          <FormCard title="اطلاعات واحد">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <FormInput control={form.control} name="name" label="نام واحد" required />
              <FormSelect control={form.control} name="type" label="نوع واحد" options={unitTypeOptions} required />
            </div>
          ...
          <FormCard title="اطلاعات تماس">
            <FormInput control={form.control} name="address" label="آدرس" />
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <FormInput control={form.control} name="phone" label="تلفن" />
          ...
          <FormCard title="ویژگی‌های انبار">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <FormInput control={form.control} name="warehouseCapacity" label="ظرفیت انبار (متر مربع)" type="number" />
              <FormInput control={form.control} name="fleetSize" label="تعداد ناوگان" type="number" />
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <FormCheckbox control={form.control} name="hasColdStorage" label="دارای سردخانه" />
              <FormInput control={form.control} name="serviceRadius" label="شعاع سرویس (کیلومتر)" type="number" />
            </div>
          </FormCard>
