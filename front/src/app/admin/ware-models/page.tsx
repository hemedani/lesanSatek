...
import { gets } from "@/app/actions/wareModel/gets"
import { gets as getWareTypes } from "@/app/actions/wareType/gets"
import { count } from "@/app/actions/wareModel/count"
import { fetchWareCounts } from "@/lib/ware-counts"
import { WareModelsClient } from "./ware-models-client"
import type { WareModel } from "./ware-models-client"

const LIMIT = 20

const WARE_MODEL_PROJECTION = {
  _id: 1,
  name: 1,
  enName: 1,
  createdAt: 1,
...
export default async function WareModelsPage({
  ...
  const [listResult, typesResult, countResult, counts] = await Promise.all([
    gets(listSet, WARE_MODEL_PROJECTION),
    getWareTypes({ activeRoleId, page: 1, limit: 100 }, { _id: 1, name: 1 }),
    count({ activeRoleId, ...(search ? { name: search } : {}) }),
    fetchWareCounts(),
  ])

  const items = (listResult.success ? listResult.body || [] : []) as WareModel[]
  ...
  const prevPageUrl = page > 1 ? `/admin/ware-models?page=${page - 1}${qs ? `&${qs}` : ""}` : ""
  const nextPageUrl = page < totalPages ? `/admin/ware-models?page=${page + 1}${qs ? `&${qs}` : ""}` : ""

  return (
    <WareModelsClient
      items={items}
      wareTypes={wareTypes}
      countsByModel={counts.byModel}
      prevUrl={prevPageUrl}
      nextUrl={nextPageUrl}
      page={page}
