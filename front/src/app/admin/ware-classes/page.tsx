...
import { gets } from "@/app/actions/wareClass/gets"
import { gets as getWareTypes } from "@/app/actions/wareType/gets"
import { count } from "@/app/actions/wareClass/count"
import { fetchWareCounts } from "@/lib/ware-counts"
import { WareClassesClient } from "./ware-classes-client"
import type { WareClass } from "./ware-classes-client"

const LIMIT = 20

const WARE_CLASS_PROJECTION = {
  _id: 1,
  name: 1,
  enName: 1,
  createdAt: 1,
...
export default async function WareClassesPage({
  ...
  const [listResult, typesResult, countResult, counts] = await Promise.all([
    gets(listSet, WARE_CLASS_PROJECTION),
    getWareTypes({ activeRoleId, page: 1, limit: 100 }, { _id: 1, name: 1 }),
    count({ activeRoleId, ...(search ? { name: search } : {}) }),
    fetchWareCounts(),
  ])

  const items = (listResult.success ? listResult.body || [] : []) as WareClass[]
  ...
  const prevPageUrl = page > 1 ? `/admin/ware-classes?page=${page - 1}${qs ? `&${qs}` : ""}` : ""
  const nextPageUrl = page < totalPages ? `/admin/ware-classes?page=${page + 1}${qs ? `&${qs}` : ""}` : ""

  return (
    <WareClassesClient
      items={items}
      wareTypes={wareTypes}
      countsByClass={counts.byClass}
      prevUrl={prevPageUrl}
      nextUrl={nextPageUrl}
      page={page}
